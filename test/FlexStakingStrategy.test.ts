                    ...
                    describe(`startDay ${startDay}`, () => {
                      ...
                      describe(`Has other tokens`, () => {
                        ...
                        beforeEach(async () => {
                          ...
                          await item
                            .connect(user)
                            .mint(1, stakingStrategy.address, token.address, '0x')
                          // tested token
                          await item
                            .connect(user)
                            .mint(mintAmount, stakingStrategy.address, token.address, '0x')

                          tokenPrice = await item.tokenPrice(tokenId)
                        ...
                        it(`Regular: claim every period (min lock). startDay=${startDay}, mintAmount=${mintAmount}`, async () => {
                          ...
                          for (let i = 0; i < minMonthsCount; i++) {
                            ...
                            if (i <= initialMonths) {
                              ...
                              estimatedRewards = await treasury.usdAmountToToken(
                        ...
                        it(`Regular: claim every period (max lock). mintAmount=${mintAmount}`, async () => {
                          for (let i = 0; i < daysDiff; i++) {
                            console.log(`IIIII ${i}`)
                            await time.increase(24 * 60 * 60)
                            console.log(`initial days ${i} ${await getDate()}`)
                        ...
                        it(`Regular: claim all in one (max lock - 1). mintAmount=${mintAmount}`, async () => {
                          for (let i = 0; i < daysDiff; i++) {
                            console.log(`IIIII ${i}`)
                            await time.increase(24 * 60 * 60)
                            console.log(`initial days ${i} ${await getDate()}`)
                          ...
                          let estimatedRewards = BigNumber.from('0')
                          const claimMouthsCount = maxMonthsCount - 1
